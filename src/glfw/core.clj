...
(defn get-clipboard-string
  ...
  * The returned string is valid only until the next call to get-clipboard-string or set-clipboard-string, or until the library is terminated.
...
(defn get-joystick-buttons
  ...
  For backward compatibility with earlier versions that did not have GetJoystickHats, the button array also includes all hats, each represented as four buttons. The hats are in the same order as returned by get-joystick-hats and are in the order up, right, down and left. To disable these extra buttons, set the joystick-hat-buttons init hint before initialization.
...
(defn get-key-scancode
  ...
  If the key is key-unkown or does not exist on the keyboard this method will return -1.
...
(defn set-char-mods-callback
  ...
  The character with modifiers callback is intended for implementing custom Unicode character input. For regular Unicode text input, see SetCharCallback. Like the character callback, the character with modifiers callback deals with characters and is keyboard layout dependent. Characters do not map 1:1 to physical keys, as a key may produce zero, one or more characters. If you want to know whether a specific physical key was pressed or released, see set-key-callback instead.
...
(defn set-time
  ...
  floor((2^64 - 1) / 10^9)
  and is due to implementations storing nanoseconds in 64 bits. The limit may be increased in the future.
...
(defn set-window-size-limits
  ...
  * This function must only be called from the main thread.
  ...
  #Parameters
  1. window - the window to set limits for
  2. minwidth - the minimum width, in screen coordinates, of the client area, or dont-care
  3. minheight - the minimum height, in screen coordinates, of the client area, or dont-care
  4. maxwidth - the maximum width, in screen coordinates, of the client area, or dont-care
  5. maxheight - the maximum height, in screen coordinates, of the client area, or dont-care"
...
(defn window-hint
  ...
  The hints, once set, retain their values until changed by a call to glfwWindowHint or DefaultWindowHints, or until the library is terminated.
